...
// Horrible hack: slicing out the part where music actually plays, so we can remove SDL. 
// This is all in service to a low-bullshit version of the Doom source that "just works." 
// And by "just works" I mean it compiles, and runs in memory, but has no interface. 

// That was straightforward, if weird. I got it to compile without SDL, and... music played. 
// Had it set to OPL. 
// Which I guess I'll also have to remove? Just rip that out, though. 
// No sense having two janky systems we aren't really using. One is plenty. 
// On the plus side the GUS instrumentation is interesting. 
// E1M8's not quite correct, but it's better than how Windows ME butchered it. 
...
// Crap I can slice out without breaking vanilla Doom music. 
//#define SDL_EXTRA
...
#endif 		// #ifdef SDL_EXTRA - slice out Vorbis / FLAC support
...
{
#ifdef SDL_MUSIC		// Just ditch it all. 
    char *filename;
//#ifdef SDL_MUSIC
    Mix_Music *music;
//#endif
    ...
    {
		// Mildly suspicious about whether this is an SDL thing. 
...
// Oddly this hasn't thrown an error yet, as I'm slicing out the SDL bits that do things. 
// Maybe because it's never called? I.e. removing the Vorbis / FLAC stuff silently elides this. 
// If this is necessary I guess I'd just return 0.0. 
